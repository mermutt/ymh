...
Revision: Rev 8 (WITHDRAW the Rev 7 context-bar provider-window fallback:
        ...
        the `context.show` snapshot or
        `config.agent.compaction.context_window_tokens`; a correct per-model
        default is deferred to a future spec-08 change). Rev 7 (context-bar provider-window fallback, 25-D7: adds the provider's
        advertised `ProviderCapabilities::max_context_tokens` as the final window
        source, applied daemon-side in `HostRuntime::showContext`; no wire change
        and no other decision touched). Rev 6 (F1 traceability amendment: pinned `set(const Session&, …)`,

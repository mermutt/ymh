...
```
Status: draft (Rev 1) — NOT reviewed. Design only; no code may be written for
...
Supersedes: `00-architecture.md` §55 (line 4889) "Cordis-compatible
        configuration" — **narrowed**, not removed: the *agent-preset
        composition* layer is adopted; Cordis configuration, the plugin
        dependency graph, and hot module replacement remain omitted. See §1.3.
        It also realizes `00-architecture.md` §38 "Profiles" (line 3986) with a
        concrete mechanism, and narrows `00 §3` Non-Goals (line 195) for the
        same layer only.
```
...
|---|---|---|---|
| 00 | `00-architecture.md` §55 `:4889` ("Cordis-compatible configuration" deliberate omission) | **Narrows** the omission for the *agent-preset layer only*. Preset composition semantics are adopted; Cordis config, plugin dependency graph, and hot reload stay omitted. | The user asks ymh to follow dsh's agent handling; spec 42 already adopted the semantics without Cordis config. |
...
| 00 | `00-architecture.md` §3 Non-Goals `:195` | **Narrows** for the agent-preset layer: capability compositions are in; "complex workflow authoring" and "full Cordis compatibility" remain out. | Same as above. |
...
| 46 | `46-permissions-ui-errata.md` D12 (`:2467-2780`) | **Extends**: D12's mapping is retained in full; 52-D7/D8 add the all-providers/all-profiles mapping and correct the forced-`muse-glimmer` defect. | The user wants all endpoints/models, not just the default. |
...
`YMH_AGENT_MODEL`, `--model`, or the `agent.model` config key
(`src/config/config.cpp:1364-1366`, `src/cli/cli.cpp:128-130`). The CLI also
overrides `llm.provider`/`llm.base_url`/`llm.api_key_env` on the single endpoint
(`src/cli/cli.cpp:131-138`).
...
global (required) → workspace (optional) → `YMH_*` env → CLI. Scalars are
...
- **`src/config/config.cpp`** owns the pure mapping:
  ...
  and the helpers `map_localcode_mcp_server`, `map_localcode_tool`,
  `is_valid_localcode_decision`, `escape_localcode_reference` (`:1424-1458`).
...
- **`src/cli/cli.cpp`** owns the CLI-side helpers:
  ...
  `print_localcode_notes` (`:742`), `validate_imported_mcp` (`:768`),
  `write_imported_config` (`:851`), and `maybe_import_localcode_config`
  (`:944-1000`), invoked at `:1183`.
...
include_runtime_context }` (`:16-21`) and `register_persona` (`:32`). The shipped
default (`src/prompt/persona.cpp:12-17`) is **byte-identical to dsh's `standard`
...
- `models.<name>.endpoint` is required and must name an existing endpoint
  (52-I2). `models.<name>.model` is required and non-empty.
...
struct ResolvedModel {
    ...
    std::string                  source;          // "agent.model" | "llm.active_model" |
...
2. **`llm.active_model`** — **name only**; must resolve to a model entry, else
   `ConfigError` (52-F1).
3. **the default endpoint's `model`** — `llm.default.model` (or flat `llm.model`
   when no `llm.default` object exists); a literal wire id on the default
   endpoint. Backward-compatible path.
4. **built-in `"deepseek-flash"`** on the built-in default endpoint.

The **default endpoint** is the merge of the built-in defaults, the flat `llm.*`
fields, and the `llm.default` object (nested wins, per the retained `21 §7.7`
quirk). A model entry's endpoint overrides the default endpoint's connection
fields for that selection only.
...
- `llm.endpoints` and `llm.models` are **maps**; across layers they merge
  **per entry** (last writer wins for a colliding entry name), unlike arrays
  which replace wholesale. This is the one new merge semantic; it is additive
  and does not change any existing key's merge.
...
- The redaction rule (`46-D12.11`) is unchanged and already matches nested
  `"api_key"` keys; no new secret is introduced, only more of the same key.
...
- **52-I1 (name well-formedness).** Every endpoint/model name is non-empty,
  ...
  grammar check (52-F5).
- **52-I2 (no dangling endpoint).** Every `llm.models.<n>.endpoint` names an
  existing `llm.endpoints` entry; resolution fails at load, never at request
  time (52-F2).
- **52-I3 (total resolution).** `resolve_model` returns a `ResolvedModel` with a
  non-empty `model_id` and a populated endpoint for every valid config; it is
  deterministic and depends only on `Config` (52-I5, 52-F1).
- **52-I4 (secret containment).** A literal `api_key` is read only from the
  global layer and only from a file with no group/other bits; it is never
  logged, rendered, or event-logged. Guard: the existing `0600` + global-only
  checks, extended to every endpoint (52-F4).
...
Component-local tags `52-F#`, each an instance of the shared `00 §54` findings.
...
- **52-F3: name collision across layers.** A model/endpoint name is defined in
  ...
  yields the wrong connection. **Guard:** per-entry last-writer-wins is pinned
  (52-D4), and the resolved name is reported by `resolve_model.source`.
  Instance of F2.
- **52-F4: literal key leak.** An endpoint `api_key` is placed in the workspace
  layer or the global file is group/world-readable. **Guard:** 52-I4 / 46-D12.2.
  Instance of F1.
...
- **52-F6: ambiguous `agent.model`.** A literal wire id that also happens to
  ...
  user who meant the literal id. **Guard:** 52-D3 pins name precedence; the
  resolution `source` is logged. Instance of F2.
- **52-F7: silently dropped provider.** A `bedrock` (or other unknown type)
  ...
  **Guard:** 52-D7 prints a note for every skipped provider. Instance of F1.
...
| `base_url` (http/https) | `llm.endpoints.<name>.base_url` | non-http(s) → **skip the endpoint with a note** (existing `is_http_url`, `:1460-1462`) |
...
| any other `type` | — | skipped with a note |
| missing `type` | — | skipped with a note |
...
|---|---|---|
| `providers.<n>.type` | `llm.endpoints.<n>.provider` | `openai-compat`/`openai-compatible` → `openai-compatible`; `bedrock`/other skipped with a note |
| `providers.<n>.base_url` | `llm.endpoints.<n>.base_url` | http(s) only |
| `providers.<n>.api_key` | `llm.endpoints.<n>.api_key` | literal; global 0600 |
| `profiles.<p>.provider` | `llm.models.<p>.endpoint` | dangling → skip model with note |
| `profiles.<p>.model` | `llm.models.<p>.model` | required |
| `profiles.<p>.max_tokens` | `llm.models.<p>.max_tokens` | `> 0` |
...
| `default_profile` | `llm.active_model` | if imported |
...
| `mcp_servers` | `mcp_servers` | unchanged (25-D13) |
...
- 52 adds **row fields**: `persona` (an object with
  `prefix`/`suffix`/`complete`/`include_runtime_context`), `permission_preset`,
  and `model` (optional; see 5.7). The legacy flat `persona_prefix`/
  `persona_suffix` fields are retained; `persona` is additive.
...
|---|---|---|
| `minimal` | `minimal` | **shipped**: single-tool composition (persistent shell), persona `complete`, runtime context suppressed, compaction absent |
| `standard` | `standard` | **shipped**: all ymh tools, dsh's standard persona, instructions enabled, compaction on |
...
verbatim is open question 52-OQ-1; the recommendation is to keep them (the user
...
error, not a silent empty composition (52-F11).
...
- **minimal's single tool.** dsh minimal allows only a *persistent* shell
  ...
  The shipped `minimal` therefore allows `shell` (always available); if the PTY
  is available the same preset may additionally allow `terminal` via an
  `include_shipped_root`-local override, but the shipped file pins `shell` so
  the preset is total.
- **minimal's persona.** dsh's prefix is `"You are a helpful software engineer
  ...
  the reason for the `persona.complete` / `include_runtime_context` row
  extension (§5.5).
- **standard's persona.** ymh's shipped default persona is already byte-identical
  (`src/prompt/persona.cpp:12-17`); the preset row restates it so the preset is
...
- **`ptc`/`cordis`.** Reserved ids with no `preset.jsonc`; `resolve()` throws
  `PresetUnavailable` (a new subclass of `PresetError`, §5.6). They are listed
  by `list()` so the user sees why they cannot be mounted.

### 5.4 Decision 52-D13: the shipped-presets root

`default_presets_shipped_root()` (`src/agent/preset.cpp:307-309`) returns `{}`
today, so `roots()` skips it (`:322-327`). 52 pins a real path:
...
// which case the shipped root contributes nothing (and 52-F18 warns once).
...
- Discovery order is shipped → user (`~/.config/ymh/presets`) → `presets.root`,
  ...
  overriding a shipped id is a duplicate-id failure (52-OQ-10).
...
- **Runtime context.** `include_runtime_context == false` suppresses the
  runtime-context snapshot for that preset's sessions. Today the runtime
  context is registered **globally** at `src/agent/workspace_runtime.cpp:190-196`
  (not scoped), so a per-preset suppression needs a scoped registration: the
  runtime context becomes a **scope-level context** registered under the
  preset's standing scope, and the global registration is used only when no
  preset is mounted. Interface:
  ...
  // PersonaRow::include_runtime_context.
  ```
...
struct PresetRow {
    ...
    std::string                    group;
    bool                           disabled = false;
    std::optional<PersonaRow>      persona;            // NEW; supersedes the pair below
...
struct EndpointSettings {                 // 52-D1
    ...
    std::vector<std::pair<std::string, std::string>> headers;  // non-secret
    std::size_t                max_concurrency = 4;
    std::chrono::milliseconds  connect_timeout{10'000};
    std::chrono::milliseconds  idle_timeout{60'000};
    std::chrono::milliseconds  request_timeout{120'000};
...
- **52-I10 (scoped suppression).** `include_runtime_context == false` suppresses
  the runtime-context snapshot only for the preset's sessions, never globally.
...
- **52-F15: persona duplicate in one layer.** Two rows define
  `deployment:persona-prefix` in the same scope; the registry throws
  (`36 §2.1`). **Guard:** 42's existing duplicate-name check; `persona` is one
  row per preset in the shipped files. Instance of F2.
- **52-F16: shipped root missing.** The install does not place the shipped
  presets, so `standard` is unknown and the default is absent. **Guard:**
  52-F18 warns once at startup; `resolve(nullopt)` with no default still fails
  loud (42-F1). Instance of F1.
...
- **52-F18: shipped root absent warning.** A build with no `YMH_PRESETS_DIR`
  silently has no shipped presets. **Guard:** one warning at roster load when
  `include_shipped_root` is true and the root is empty. Instance of F1.

---

## 6. Invariants (consolidated)

Part A: 52-I1..52-I7. Part B: 52-I8..52-I14. All extend `42-I1..42-I12` and
`26-I1..26-I12`. Every `52-F#` names the shared `00 §54` finding it instantiates.

## 7. Failure modes (consolidated)

`52-F1..52-F18`, contiguous. Coverage matrix in §9.5.
...
| `minimal` | `dsh-agent-presets/presets/minimal` | persona `complete`/no runtime context; single persistent shell → ymh `shell`. |
| `standard` | `presets/standard` | all tools; dsh persona; `agent-instructions` maxBytes 65536. |
...
- `Import52_MuseProfileOnlyForMuse`: `Muse-Glimmer-30B` gets
  ...
  none (52-I7, 52-F8).
- `Preset52_Roster`: `list()` returns `minimal`, `standard`, `ptc`, `cordis`;
  `resolve("ptc")` throws `PresetUnavailable` (52-I8, 52-F11).
...
- A two-endpoint live run against the user's `itg` and `ted-ai` providers
  (skipped without keys), asserting the selected model reaches the right
  `base_url`. Existing live tests stay opt-in.

### 9.5 Failure-mode coverage

Every `52-F#` has at least one named test above; the matrix is completed when
...
1. **52-OQ-1 — mirror dsh preset names verbatim?** Recommendation: yes
   (`minimal`/`standard`/`ptc`/`cordis`); `ptc`/`cordis` reserved. Alternative:
   ymh-specific names (`basic`/`full`) with dsh ids as aliases.
...
8. **52-OQ-8 — retain the `llm.default` flat-key quirk?** Recommendation: retain
   (`21 §7.7` J12); fixing it is a separate semantic change.
...
10. **52-OQ-10 — user override of a shipped preset id.** A duplicate id across
    roots fails load (42-F8); a user cannot currently shadow a shipped preset
    with the same id. Whether to allow user-root shadowing is open.
...
12. **52-OQ-12 — CLI/env surface.** `--endpoint`, `YMH_LLM_ACTIVE_MODEL`,
    `YMH_LLM_ENDPOINT` are proposed; the final spelling needs a UI-spec review
    (spec 10/25).
